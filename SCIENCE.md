# The Science Behind Momentum

## Executive Dysfunction and Depression

Depression is reliably associated with impairments in executive function -- the mental processes responsible for planning, initiating, and completing goal-directed activities. A large meta-analysis of 113 studies found effect sizes of d=0.32-0.97 across multiple executive domains (inhibition, shifting, working memory, planning, and fluency). These deficits are linked to reduced prefrontal cortex activity, particularly in the dorsolateral prefrontal cortex, which is critical for organising behaviour toward goals. Fatigue and energy loss appear to be the primary depression symptoms bridging mood and executive impairment, which is why even simple tasks can feel insurmountable.

- [Snyder (2013) -- Meta-analysis of EF in MDD, Psychological Bulletin](https://doi.org/10.1037/a0028727)
- [Hagen et al. (2023) -- EF deficits by depression severity, Frontiers in Psychiatry](https://doi.org/10.3389/fpsyt.2023.1225062)
- [Van Niekerk et al. (2023) -- Fatigue bridges depression and EF, CPNEC](https://doi.org/10.1016/j.cpnec.2023.100189)

## How Momentum Helps: Behavioural Activation

Momentum's core design -- breaking tasks into small manageable steps, scheduling activities, and tracking completion -- is grounded in Behavioural Activation (BA), one of the most well-supported psychological treatments for depression. BA works by reversing the vicious cycle of inactivity and low mood: when depression leads to withdrawal, the loss of positive reinforcement deepens the depression further. By encouraging small, achievable actions, BA restores a sense of mastery and pleasure. A meta-analysis of 20 randomised controlled trials found BA produced a pooled effect size of g=0.52 on well-being, comparable to other leading psychological interventions.

- [Mazzucchelli et al. (2010) -- BA for well-being meta-analysis, J Positive Psychology](https://doi.org/10.1080/17439760903569886)
- [Dimidjian et al. (2006) -- BA vs CBT vs medication for depression, JCCP](https://doi.org/10.1037/0022-006X.74.4.658)

## Focus Timers and Structured Work

Momentum's 15-minute focus timer draws on the principles behind the Pomodoro Technique, which structures work into focused intervals followed by short breaks. Research with 87 university students found that students taking systematic timed breaks reported lower fatigue, less distractedness, and higher concentration and motivation compared to those who self-regulated their breaks. For people with executive dysfunction, this external structure is especially valuable -- it removes the cognitive burden of deciding when to work and when to rest, effectively offloading an executive function that depression impairs.

- [Biwer et al. (2023) -- Pomodoro vs self-regulated breaks, British J of Ed Psychology](https://doi.org/10.1111/bjep.12593)

## Gentle Encouragement and Self-Compassion

Momentum's encouragement messages reflect the principles of self-compassion -- treating yourself with kindness during difficulty rather than self-criticism. A meta-analysis of 56 randomised controlled trials found that self-compassion interventions produced moderate reductions in depressive symptoms (g=0.66) and anxiety (g=0.57). Crucially, self-compassion has been shown to increase behavioural activation: people who are kinder to themselves after setbacks are more motivated to try again, creating a positive feedback loop with task completion.

- [Wilson et al. (2023) -- Self-compassion reduces depression/anxiety meta-analysis, Mindfulness](https://doi.org/10.1007/s12671-023-02148-x)
- [Ferrari et al. (2019) -- Self-compassion interventions meta-analysis, Mindfulness](https://doi.org/10.1007/s12671-019-01134-6)
- [Adie et al. (2021) -- Self-compassion, activation and depression, Mindfulness](https://doi.org/10.1007/s12671-021-01637-1)

## BIS/BAS Motivational Personalization

Momentum now includes a BIS/BAS profile (Behavioural Inhibition / Behavioural Activation) to personalise support. The BIS/BAS framework captures meaningful individual differences in threat-sensitivity (BIS) and reward/goal responsiveness (BAS). In practice, this helps explain why one user may need lower-pressure initiation cues and shorter focus blocks, while another benefits more from reward-framed nudges and longer persistence blocks.

In Momentum, this is used in a lightweight way: profile-informed defaults adjust focus/break length and encouragement style without changing the underlying task model. The goal is not diagnosis, but reducing activation friction by matching prompts to motivational style.

- [Carver & White (1994) -- BIS/BAS scales, Journal of Personality and Social Psychology](https://doi.org/10.1037/0022-3514.67.2.319)

## Accessibility, Reduced Visual Load, and Interface Friction

Executive dysfunction is often worsened by excessive UI complexity, high decision overhead, and visually noisy interfaces. Momentum's light/dark mode, higher-contrast option, larger text, reduced-visual-load mode, and bottom mobile navigation are intended to lower interaction cost and improve readability under fatigue.

These choices align with established accessibility and usability guidance: strong contrast and readable typography improve perceptual clarity, and predictable navigation reduces cognitive switching costs. In short, interface simplicity is treated as a therapeutic support feature, not just visual polish.

- [W3C WCAG 2.2 -- Contrast and readable presentation guidance](https://www.w3.org/TR/WCAG22/)
- [Nielsen Norman Group -- Recognition over recall and consistency in navigation](https://www.nngroup.com/articles/ten-usability-heuristics/)

## Nature Imagery and Mood

The peaceful nature images Momentum displays at launch are not merely decorative. Viewing images of natural environments has been shown to increase feelings of comfort and relaxation with a large effect size, while reducing activity in the orbitofrontal cortex -- a brain region that is hyperactive in depression and anxiety. Importantly, people with more depressive symptoms benefit more from viewing nature than those without, making this a particularly fitting intervention for Momentum's users.

- [Yamashita et al. (2021) -- Nature images improve mood via OFC, IJERPH](https://doi.org/10.3390/ijerph18105500)
- [Matsubara et al. (2024) -- Nature images in depression/anxiety patients, J Affective Disorders](https://doi.org/10.1016/j.jad.2024.04.012)
- [Beute & de Kort (2021) -- Depressive symptoms predict greater nature benefit, J Env Psychology](https://doi.org/10.1016/j.jenvp.2021.101601)

## A Note

Momentum is a supportive tool, not a replacement for professional treatment. If you are experiencing depression, please reach out to a healthcare provider. The strategies in this app are drawn from evidence-based approaches, but they work best as part of a broader support system.
