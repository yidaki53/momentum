...
def print_status(summary: StatusSummary) -> None:
    ...
    today = summary.today
    total_today = (
        today.tasks_completed + len(summary.pending_tasks) + len(summary.active_tasks)
    )
    ...
    lines: list[str] = [
        ...
        "",
        (
            f"This week: {summary.week_tasks_completed} tasks, "
            f"{summary.week_focus_minutes} min focused"
        ),
